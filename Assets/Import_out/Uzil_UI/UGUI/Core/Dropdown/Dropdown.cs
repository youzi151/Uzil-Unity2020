...
public class DropdownItem : MonoBehaviour, IPointerEnterHandler, ICancelHandler
{
	...
	public TextExt         text          { get { return m_Text;          } set { m_Text = value;           } }
...
public class DropdownOptionData
{
	...
	public DropdownOptionData(string text)
	{
		this.text = text;
	}

	public DropdownOptionData(Sprite image)
	{
		this.image = image;
	}
	...
	public DropdownOptionData(string text, Sprite image)
	{
		this.text = text;
		this.image = image;
	}
...
public class Dropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
{
	...
	{
		[SerializeField]
		private List<DropdownOptionData> m_Options;

		/// <summary>
		/// The list of options for the dropdown list.
		/// </summary>
		public List<DropdownOptionData> options { get { return m_Options; } set { m_Options = value; } }


		public OptionDataList()
		{
			options = new List<DropdownOptionData>();
		}
	}
	...
	/// </summary>
	public class DropdownEvent : UnityEvent<int, DropdownOptionData, DropdownItem> {}
	...
	[SerializeField]
	private TextExt m_CaptionText;

	/// <summary>
	/// The Text component to hold the text of the currently selected option.
	/// </summary>
	public TextExt captionText { get { return m_CaptionText; } set { m_CaptionText = value; RefreshShownValue(); } }
	...
	[SerializeField]
	private TextExt m_ItemText;

	/// <summary>
	/// The Text component to hold the text of the item.
	/// </summary>
	public TextExt itemText { get { return m_ItemText; } set { m_ItemText = value; RefreshShownValue(); } }
	...
	/// </code>
	/// </example>
	public List<DropdownOptionData> options
	{
	...
	private static DropdownOptionData s_NoOptionData = new DropdownOptionData();
	...
	void Set(int value, bool sendCallback = true)
	{
		...
		if (sendCallback)
		{
			...
			m_OnValueChanged.Invoke(m_Value, this.options[m_Value], this.m_Items[m_Value]);
		}
	}

	/** 是否自動捲動到當前選項 */
	[SerializeField]
	public bool isAutoScrollToCurrent = true;
	...
	{
		DropdownOptionData data = s_NoOptionData;
	...
	/// </remarks>
	public void AddOptions(List<DropdownOptionData> options)
	{
	...
	public void AddOptions(List<string> options)
	{
		for (int i = 0; i < options.Count; i++)
			this.options.Add(new DropdownOptionData(options[i]));
		RefreshShownValue();
	}
	...
	public DropdownOptionData GetOption (int idx) {
		if (idx < 0 || idx > this.options.Count) return null;
		return this.options[idx];
	}

	public DropdownItem GetItem (int idx) {
		if (idx < 0 || idx > this.m_Items.Count) return null;
	...
	public void Show()
	{
		...
		for (int i = 0; i < options.Count; ++i)
		{
			DropdownOptionData data = options[i];
		...
		if (this.isAutoScrollToCurrent) {
			RectTransform valItemTrans = m_Items[this.m_Value].rectTransform;
			Vector2 contentPos = contentRectTransform.anchoredPosition;
			contentPos.y = contentRectTransform.sizeDelta.y - (valItemTrans.anchoredPosition.y + (valItemTrans.pivot.y * valItemTrans.rect.height));
			contentRectTransform.anchoredPosition = contentPos;
		}
	...
	private DropdownItem AddItem(DropdownOptionData data, bool selected, DropdownItem itemTemplate, List<DropdownItem> items)
	{
		...
		item.toggle.interactable = data.isInteractable;
